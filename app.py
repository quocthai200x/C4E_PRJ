...
def tntc():
  if 'logged' not in session:
    ...
    if request.method=="GET":
      ...
      form=request.form
      if len(form) < 5:
        return render_template('tracnghiemtc.html',id_login=session['id_login'],user=session['user'],login=session['logged'],cauhoi=cauhoi_all,check='False')
      else:
        chon=form['1']+form['2']+form['3']+form['4']
        return redirect('/tracnghiemtc/{}'.format(chon))

@app.route('/tracnghiemtc/<ma>')
def mae(ma):
  nd=dapan_data.find_one({'codeid':ma})
  return render_template('tracnghiem.html',nd=nd,id_login=session['id_login'],user=session['user'],login=session['logged'])
